...
export default async function ArchivedJobsPage() {
  ...
  const jobs = await prisma.jobListing.findMany({
    where: { employerId: user.id, OR: [{ isPublished: false }, { status: { not: 'OPEN' } }] },
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/employer/jobs/new" className="px-4 py-2 rounded bg-blue-600 text-white">New job</Link>
        </div>
          ...
          {jobs.map(j => (
            <div key={j.id} className="p-4 flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                ...
                <Link href={`/employer/jobs/${j.id}/applicants`} className="text-blue-600 text-sm hover:underline">View applicants</Link>
              </div>
            </div>
          ))}
          {jobs.length === 0 && <div className="p-6 text-sm text-gray-600">No archived jobs.</div>}
        </div>
      </div>
    </div>
  )
