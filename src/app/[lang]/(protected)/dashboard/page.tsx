...
export default async function DashboardPage({
  ...
  try {
    ...
    const res = await fetch(`${base}/api/posts?limit=8`, {
      cache: "no-store",
      headers: { cookie },
    });
    if (res.ok) {
      const data = await res.json();
      posts = (data.posts || []) as PostShape[];
      initialCursor = data.nextCursor || null;
      trending = (data.trending || []) as PostShape[];
    }
  } catch {}
      ...
      <div className="container mx-auto px-4 max-w-screen py-6">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <FeedClient
              initialPosts={posts as PostShape[]}
              user={user as User}
              initialCursor={initialCursor}
              trendingPosts={trending as PostShape[]}
            />
          </div>
