...
export function PostCard({ post, user }: PostCardProps) {
  ...
  const images: string[] = useMemo(() => post.images ?? [], [post.images]);
  ...
  const handleLike = async () => {
    try {
      ...
      if (response.ok) {
        const data: { liked?: boolean } = await response
          .json()
          .catch(() => ({}) as { liked?: boolean });
        const nextLiked =
        ...
        try {
          ...
          (await import("@/lib/socket")).socketManager.emit(
            "post_liked",
            payload,
          );
        } catch {}
  ...
  const handleComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        let data: { comment?: PostComment } | null = null;
        try {
          data = await response.json();
        } catch {
          data = null;
        }
        const created: PostComment | null = data?.comment ?? null;
        if (created) {
  ...
  return (
    <>
      <Card className="bg-white shadow-sm">
        <CardContent className="p-6">
          ...
          <div className="flex items-start space-x-3 mb-4">
            <Link
              href={`../en/user/${post.author.id}`}
              className="shrink-0"
              aria-label={`${post.author.name}'s profile`}
            >
              <Avatar className="h-10 w-10">
                <AvatarImage
                  src={getAvatarUrl(post.author.image, post.author.name)}
                />
          ...
          <div className="mb-4">
            ...
            {post.type === "IMAGE" && images.length > 0 && (
              <div
                className={`grid ${images.length === 2 ? "grid-cols-2" : images.length >= 3 ? "grid-cols-3" : "grid-cols-1"} gap-2 mt-2`}
              >
              ...
              <div className="mt-3 p-3 border border-blue-100 rounded-lg bg-blue-50">
                <div className="flex items-center gap-2 min-w-0 max-w-full">
                  ...
                  <a
                    ...
                    className="break-all text-blue-700 text-sm underline"
                  >
                    {post.linkUrl}
                  </a>
                </div>
            ...
            {/* Video chip */}
            {post.videoUrl && (
              <div className="mt-3 p-3 border border-purple-100 rounded-lg bg-purple-50">
                <div className="flex items-center gap-2 min-w-0 max-w-full">
                  <svg
                    ...
                    fill="currentColor"
                  >
                    <path d="M17 10.5V7a2 2 0 0 0-2-2H5C3.895 5 3 5.895 3 7v10c0 1.105.895 2 2 2h10a2 2 0 0 0 2-2v-3.5l4 4v-11l-4 4z" />
                  </svg>
                  <a
                    href={post.videoUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="break-all text-purple-700 text-sm underline"
                  >
                    {post.videoUrl}
                  </a>
                </div>
      ...
      <Dialog
        open={reportOpen}
        onOpenChange={(o) => !reporting && setReportOpen(o)}
      >
      ...
      <AlertDialog
        open={blockOpen}
        onOpenChange={(o) => !blocking && setBlockOpen(o)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
