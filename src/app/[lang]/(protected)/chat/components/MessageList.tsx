...
import { Download, Edit, MoreHorizontal, Trash2, Image as ImageIcon, File as FileIcon, Video, Music, FileText, Reply } from 'lucide-react';
...
export function MessageList({
  ...
  return (
    <div ref={listRef} className="flex-1 space-y-6 overflow-y-auto px-4 py-6 sm:px-8">
      {loadingMore && (
      ...
      {groupedMessages.length === 0 ? (
        <div className="flex h-full flex-col items-center justify-center text-center text-slate-500">
          ...
          <p className="text-base font-semibold text-slate-700">Start something great</p>
          <p className="max-w-sm text-sm leading-relaxed text-slate-500">
            Share updates, ask questions, or send a warm hello to keep the conversation moving.
          </p>
        </div>
      ) : (
        groupedMessages.map((message, index) => {
          ...
          const currentDate = dayFormatter.format(new Date(message.createdAt));
          const previousDate = previous ? dayFormatter.format(new Date(previous.createdAt)) : null;
          const showDayChip = !previous || currentDate !== previousDate;
          const readReceipt = userReadStatus[message.id] || false;
          const senderName = isMine ? 'You' : enriched.sender?.name ?? 'Colleague';
          const senderInitial = senderName.charAt(0).toUpperCase();
          const senderImage = enriched.sender?.image ?? undefined;
          const bubbleColour = isMine ? 'bg-blue-600 text-white' : 'bg-white text-slate-900 border border-white/60';

          return (
            <div key={message.id} id={`message-${message.id}`} className="flex w-full flex-col gap-3">
              {showDayChip && (
                <div className="flex justify-center">
              ...
              {message.replyTo && (
                <div className="text-xs text-slate-500 sm:ml-16">
                  Replying to: {groupedMessages.find((m) => m.id === message.replyTo)?.text ?? 'Message unavailable'}
                </div>
              )}

              <div className={`group flex w-full items-start gap-3 ${isMine ? 'justify-end' : 'justify-start'}`}>
                {!isMine && (
                ...
                >
                  <div className={`w-full rounded-2xl px-4 py-2 shadow-sm backdrop-blur transition ${bubbleColour}`}>
                    {message.text && (
                      ...
                      <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                        {message.attachments.map((attachment) => {
                          ...
                          return (
                            <div
                              key={attachment.id}
                              className={`overflow-hidden rounded-xl ${isImage ? 'border border-white/40' : 'border border-white/60 bg-white/10'}`}
                            >
                              {isImage ? (
                                <NextImage
                                  src={attachment.url}
