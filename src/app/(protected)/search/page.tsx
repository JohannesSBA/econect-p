...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    if (query) {
      void performSearch(query, type, page);
    }
  }, [searchParams]);
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (searchQuery.trim()) {
      setCurrentPage(1);
      router.push(`/search?q=${encodeURIComponent(searchQuery)}&type=${searchType}&page=1`);
    }
  };
  ...
  const handleTabChange = (value: string) => {
    ...
    setCurrentPage(1);
    if (searchQuery.trim()) {
      router.push(`/search?q=${encodeURIComponent(searchQuery)}&type=${value}&page=1`);
    }
  };
  ...
  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    if (searchQuery.trim()) {
      router.push(`/search?q=${encodeURIComponent(searchQuery)}&type=${searchType}&page=${page}`);
    }
  };
                    ...
                    <CardContent className="pt-0">
                      <div className="grid gap-3">
                        {results.people.map((person) => (
                          <Link key={person.id} href={`/${person.role === 'EMPLOYER' ? 'company' : 'user'}/${person.id}`}>
