...
export function FeedClient({
  ...
  useEffect(() => {
    const onNewPost = (payload: unknown) => {
      ...
      const data = payload as { post?: PostShape };
    ...
    const onLiked = (payload: unknown) => {
      ...
      const data = payload as { postId?: string; liked?: boolean };
      if (!data.postId || typeof data.liked !== "boolean") {
        return;
      }
      ...
      setPosts((prev) =>
        prev.map((p) =>
          p.id === data.postId
            ? { ...p, likes: updateLikes(p.likes, liked, user.id) }
            : p,
        ),
      );
    };
    const onCommented = (payload: unknown) => {
      if (!payload || typeof payload !== "object") {
        return;
      }
      const data = payload as { postId?: string; comment?: PostComment };
      if (!data.postId || !data.comment) {
        return;
      }
      const comment = data.comment as PostComment;
      setPosts((prev) =>
        prev.map((p) =>
          p.id === data.postId
