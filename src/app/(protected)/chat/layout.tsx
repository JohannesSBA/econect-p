...
export default async function ChatLayout({ children }: ChatLayoutProps) {
  ...
  if (!user) {
    return (
      <div className="flex max-h-screen items-center justify-center bg-slate-50 px-4 overflow-hidden">
        <div className="max-w-sm rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-xl">
          <h1 className="text-2xl font-semibold text-slate-900">
            User not found
          </h1>
          <p className="mt-2 text-sm text-slate-500">
            Please sign in with a valid account to continue.
          </p>
          <Link
            href="/auth/login"
  ...
  return (
    <div className="max-h-screen bg-slate-100 overflow-hidden">
      <Header user={user} />
