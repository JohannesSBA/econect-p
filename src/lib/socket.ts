class SocketManager {
  ...
  connect(userId: string) {
    ...
    this.socket.onclose = (evt) => {
      try {
        const url = (this.socket as any)?.url || wsUrl;
    ...
    this.socket.onerror = (evt) => {
      try {
        ...
        const errMsg = (evt as any)?.message || (evt as ErrorEvent)?.message || 'Connection error';
        if (!this.loggedErrorOnce) {
  ...
  emit(event: string, data: unknown) {
    ...
    if (this.socket?.readyState === WebSocket.OPEN) {
      ...
      if (this.lastUserId && (!this.socket || this.socket.readyState !== WebSocket.CONNECTING)) {
        this.connect(this.lastUserId);
      }
